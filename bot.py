...
from tracker import get_shopee_price
import requests
...
async def price(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args:
        await update.message.reply_text("❗ Please provide a Shopee product URL.")
        return

    url = context.args[0]
    title, price = get_shopee_price(url)

    if not title:
        await update.message.reply_text("⚠️ Couldn't fetch product info. Try again.")
    else:
